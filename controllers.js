...
angular.module('controller', ['songServices', 'ngResource', 'ngRoute']).controller('MainController', [
	'$scope',
	'songs',
	'socket',
	'socket-controller',
	'$resource',
	function($scope, songs, socket, socket_controller, $resource, $routeParams) {
		...
		$scope.main.lastPlayedSubmit = function($event, songName, artist, songId, albumId) {
			console.log(artist, songName, songId);
			if (songId === "") return;
			var picture = $event.target.src;
			if (picture.substr(-9,9) === "check.png") return;
			$scope.addSong({artist: artist, songName : songName, id : songId, albumId: albumId});
